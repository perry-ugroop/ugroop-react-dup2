...
import PlacesSuggestList from './PlacesSuggestList';
import PlacesSuggestItem from './PlacesSuggestItem';

const MY_API_KEY = 'AIzaSyDGYciwD6OyFLCBIBrdKQFbrkwofhJR2ac';

export class UGAutocomplete extends React.Component { // eslint-disable-line react/prefer-stateless-function
      ...
      <PlacesSuggestList>
        {suggests.length > 0
          ? suggests.map((suggest, key) => (
            <PlacesSuggestItem
              key={key}
              className={`${focusedSuggestIndex === key && 'placesSuggest_suggest-active'}`}
  ...
  render() {
    ...
    return (
      <PlacesSuggest onKeyDown={this.handleKeyDown}>
        {children}
        {search && this.state.selectedLabel !== search && this.renderSuggests()}
      </PlacesSuggest>
    );
  }
}

export default connect()(UGAutocomplete);

UGAutocomplete.propTypes = {
